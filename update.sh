...
help()
{
    echo "pull cs exist files, coffee.internal -> package: ./update.sh pull <package_dir>"
    echo "pull cs all files, coffee.internal -> package:   ./update.sh pull-all <package_dir>"
    echo "push cs all files, package -> coffee.internal:   ./update.sh push <package_dir>"
}
...
[ "$cmd" != "pull-all" ] && [ "$cmd" != "pull" ] && [ "$cmd" != "push" ] && help && exit 0
